...
Validação desta atualização: 77 testes, incluindo PostgreSQL local com as cinco migrações, isolamento de permissões, revisão concorrente, condições de turno e restauração com rollback; compilação de produção; conferência da central no navegador com dados locais e largura de celular. Em 13/09/2026, a migração 005 foi aplicada em transação no projeto de produção `sxztwtcmuhwdfoeqhiva`, com sucesso confirmado pelo SQL Editor. Verificadas as três funções públicas e RLS ativo nas três novas tabelas; histórico e sessão têm somente política SELECT, e notas privadas têm política exclusiva do Mestre. Nenhuma campanha ou ficha foi apagada na ativação.
